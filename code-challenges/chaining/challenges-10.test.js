...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc, curr) => acc + curr.reduce((a, c) => c === target ? a+=1 : a,0), 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, curr) => acc + curr.reduce((a, c) => a + c, 0), 0);
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(item => (item.gender === 'male' || item.gender === 'female')).map(ele => ele.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((acc, curr) => parseInt(curr.height) < parseInt(acc.height) ? curr : acc).name;
};
